    ...
    <div class="container">
        ...
        <el-table height="550px" border size="small" :data="data" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="资源更新" width="120" align="center">
                <template v-slot="scope">
                    <el-upload :action="updateSongImg(scope.row.id)" :show-file-list="false"
                        :on-success="handleImgSuccess" :before-upload="beforeImgUpload">
                        <el-button>更新图片</el-button>
                    </el-upload>
                    <br />
                    <el-upload :action="updateSongUrl(scope.row.id)" :show-file-list="false"
...
export default defineComponent({
    ...
    setup() {
        ...
        const breadcrumbList = computed(() => userStore.breadcrumbList);

        const isPlay = computed(() => userStore.isPlay); // 播放状态
        ...
        function setSongUrl(row) {
            userStore.setUrl(row.url)
            toggle.value = row.name;
            if (isPlay.value) {
                userStore.setIsPlay(false)
            } else {
                userStore.setIsPlay(true)
            }
        }
        // 更新歌曲图片
        ...
        function goCommentPage(id) {
            ...
            userStore.setBreadcrumbList(breadcrumbList.value)
